...
- (void)replyToHTTPRequest
{
	...
	//NSDictionary* header = [(NSDictionary*) CFHTTPMessageCopyAllHeaderFields(request) autorelease];
	
	if([method isEqualToString:@"POST"]){
		//write a file here
		
		const char* buffer = [bodyData bytes];
		int len = [bodyData length];
		
		char* fstart = strstr(buffer, "filename=\"");
		NSLog(@"fstart = %d", fstart);
		fstart += 10;
		char* fend = strstr(fstart,"\"");
		NSLog(@"fend = %d, len = %d", fend, fend - fstart);
		
		
		NSData* fileNameData = [[[NSData alloc] initWithBytes:fstart length: fend - fstart] autorelease];
		NSString* fileName = [[[NSString alloc] initWithData:fileNameData encoding:NSUTF8StringEncoding] autorelease];
		
		NSLog(@"fileName:%@",fileName);
		
		char* fcstart = strstr(fend, "\r\n\r\n");
		fcstart += 4;
		NSLog(@"fcstart = %d", fcstart);
		const char* endstr = [[NSString stringWithFormat:@"\r\n--%@--\r\n",boundary] UTF8String];
		char* fcend = (char*)(buffer + (len - strlen(endstr)));
	...
	if(!isRangeRequest)
	{
		...
		//}else{
			//CFHTTPMessageSetHeaderFieldValue(response, CFSTR("Location"), (CFStringRef)[header objectForKey:@"Referer"]);
		//}
	}
	else
	{
...
- (NSString *)filePathForURI:(NSString *)path
{
	...
	if([relativePath hasSuffix:@"/"] )
	{
		...
		if(![[NSFileManager defaultManager] fileExistsAtPath:[[url path] stringByStandardizingPath]]){
...
- (NSObject<HTTPResponse> *)httpResponseForURI:(NSString *)path
{
	...
	if([[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir])
	{
		if(!isDir){
			...
			NSMutableString* indexedString = [NSMutableString string];
			...
			for(NSString* f in files){
				BOOL d;
				if([f hasPrefix:@"__"]) continue;
				[[NSFileManager defaultManager] fileExistsAtPath:[filePath stringByAppendingFormat:@"/%@",f] isDirectory:&d];
				if(d)
					[indexedString appendFormat:@"<li><a href='%@/'>%@/</a></li>",f,f];
...
{
//	NSLog(@"data2 = %@",[[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease]);
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	...
	if(readingBody){
		...
		if([bodyData length] > [end length]){
			int start = [bodyData length] - [end length];
			char end_str[255];
			[bodyData getBytes:end_str range:NSMakeRange(start, [end length] ) ];
			end_str[[end length]] = 0;
			//char* c = (char*)(buffer + start);
			
			//printf("s = \"%s\"",end_str);
			if(strcmp(end_str, [end UTF8String])==0){
				[self replyToHTTPRequest];
				readingBody = NO;
			}
		}
		[asyncSocket readDataWithTimeout:READ_TIMEOUT tag:HTTP_REQUEST];
		/*
		[asyncSocket readDataToData:[AsyncSocket CRLFData]
						withTimeout:READ_TIMEOUT
		 //  maxLength:LIMIT_MAX_HEADER_LINE_LENGTH
	...
	else
	{
		...
		if([method isEqualToString:@"GET"] || [method isEqualToString:@"HEAD"])
		{
			...
			[boundary release];
			boundary = [[contentType substringFromIndex:[prefix length]] retain];
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	if(doneSendingResponse)
	{
		...
		if(tag == HTTP_FINAL_RESPONSE || [method isEqualToString:@"POST"])
		{
